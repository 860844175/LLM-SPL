...
def get_embedding(model,
    ...
    savefile(data1_embedding, outpath + 'vulu_embedding_' + note)
...
#         cve_desc = pd.read_csv('data/cve_desc.csv')
...
def encoding(train_df, test_df, note):
    ...
    train_enc(trainDatasetPath=trainDatasetPath)
    print("对文本进行编码")
    logging.info("对文本进行编码")
    model = TextModel().to(device)
    model.load_state_dict(torch.load('../data/encode/model.ckpt'))
